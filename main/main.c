...
static const char *TAG = "qr_scanner_example";
...
void app_main(void)
{
    ESP_LOGI(TAG, "Starting QR Scanner Example");
    ESP_LOGI(TAG, "Waiting 10 seconds for serial monitor connection...");

    // Wait 10 seconds to allow serial monitor to connect
    for (int i = 10; i > 0; i--) {
    ...
    // CRITICAL: Initialize camera BEFORE touch to avoid I2C conflicts
    ESP_LOGI(TAG, "Pre-initializing camera before touch controller...");
    esp_err_t cam_ret = camera_init();
    if (cam_ret != ESP_OK) {
        ESP_LOGE(TAG, "Camera pre-init failed: 0x%x", cam_ret);
    } else {
        ESP_LOGI(TAG, "Camera pre-initialized successfully");
        // Give camera time to stabilize before initializing other peripherals
    ...
    // Comment this line and uncomment the demo line below to switch back to widgets demo
    qr_scanner_ui_init();

    // Uncomment this to show the widgets demo instead
    // if (lvgl_lock(-1))
    // {
    //     lv_demo_widgets();
    //     lvgl_unlock();
    ...
    ESP_LOGI(TAG, "QR Scanner Example started successfully");
}
